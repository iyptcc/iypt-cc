...
from apps.tournament.models import (Origin, ScheduleTemplate, TemplateAttendance, TemplateFight, TemplateRoom,
                                    TemplateRound)
from apps.tournament.utils import _is_superior_user, _more_perm_than_group, _more_perm_than_role
...
class CuriiePreview(FormPreview):
        ...
        for line in lines:
            ...
            if len(f) >= 4:
                ...
                if orig == 'None' or orig == '':
                    orig = default
                name = "%s %s" % (f[0], f[1])
                slug = "%s-%s-%s"%(tournament.slug,slugify(orig),slugify(unidecode(name)))
                cap = _normal_capitalisation(name)
                attend = False
                user = False

                try:
                    att = Attendee.objects.get(active_user__user__username=slug,
                ...
                if f[2].lower() == 'i':
                    j = {'name': name, 'lastname': f[1], 'firstname':f[0],
                         'list': [[default,False]], 'loc': True, 'slug':slug}
    ...
    def _create_attendee(self,username,first_name,last_name,tournament):
        ...
        attendee = Attendee.objects.get_or_create(active_user=auser, tournament=tournament)[0]
    ...
    def done(self, request, cleaned_data):
        ...
        (teams, jurors) = self._dissect_currie(trn, cleaned_data['input'],
                                               cleaned_data['default'])


        memr, cre = TeamRole.objects.get_or_create(tournament=trn, type=TeamRole.MEMBER)
        leadr, cre = TeamRole.objects.get_or_create(tournament=trn, type=TeamRole.LEADER)

        prstudent, cre = ParticipationRole.objects.get_or_create(tournament=trn, type=ParticipationRole.STUDENT)
        prjuror, cre = ParticipationRole.objects.get_or_create(tournament=trn, type=ParticipationRole.JUROR)
        ...
        for t in teams:
            ...
            for me in teams[t]['m']:
                ...
                TeamMember.objects.get_or_create(attendee=attnd,
                                                 team=team, role=memr)
        ...
        for j in jurors:
            att = self._create_attendee(jurors[j]['slug'],jurors[j]['firstname'],jurors[j]['lastname'],trn)
    ...
    def process_preview(self, request, form, context):
        ...
        (teams, jurors) = self._dissect_currie(request.user.profile.tournament, form.cleaned_data['input'],
...
class PersonsPreview(FormPreview):
    ...
    def parse_params(self, request):
        ...
        person = ModelDeleteListField(queryset=Attendee.objects.filter(tournament=trn,
                                                                       **self._filters)
                                      .exclude(ex_query)
        ...
        allowedros = [r.id for r in trn.participationrole_set.all() if _more_perm_than_role(request.user, r)]

        roles = forms.ModelMultipleChoiceField(queryset=ParticipationRole.objects.filter(id__in=allowedros),required=False,
                                                widget=Select2MultipleWidget())

        allowedgrs = [gr.id for gr in trn.groups.all() if _more_perm_than_group(request.user,gr)]

        groups = forms.ModelMultipleChoiceField(queryset=Group.objects.filter(id__in=allowedgrs),required=False,
                                                widget=Select2MultipleWidget())

        team = forms.ModelChoiceField(queryset=Team.objects.filter(tournament=trn), required=False,
                                                widget=Select2Widget())

        template = forms.ModelChoiceField(queryset=trn.template_set.filter(type=Template.PERSONS), required=False,
                                                widget=Select2Widget())

        conflicting = forms.ModelChoiceField(queryset=trn.attendeeproperty_set.filter(type=AttendeeProperty.CONFLICT_ORIGINS),
                                          required=False, widget=Select2Widget)

        self.form = type("PersonsForm", (forms.Form,), {'persons':person,'roles':roles,'groups':groups,"template":template,"team":team,"conflicting":conflicting})
    ...
    def process_preview(self, request, form, context):
        ...
        if "_delete" in request.POST:
            ...
            for new in news:
                ...
                if conflict_field:
                    try:
                        apv = new.attendeepropertyvalue_set.filter(property=conflict_field).last()
                ...
                if Juror.objects.filter(attendee=new).exists():
                    ...
                    jurors.append({'new':False, 'name':new.full_name,'conflicting':list(old_confl|conflicting),'origin':ori,
                                   'experience':Juror.objects.get(attendee=new).experience})
                else:
                    try:
                        pj = new.active_user.possiblejuror_set.get(tournament=trn, approved_by__isnull=False)
                        exp = pj.experience
                    except:
                        exp = Juror.EXPERIENCE_LOW
                        pass

                    jurors.append({'new':True, 'name':new.full_name, 'origin':ori,'conflicting':list(conflicting),"experience":exp})
            ...
            for att in form.cleaned_data['persons'].prefetch_related(
                ...
                person = {"full_name": att.full_name,
    ...
    def done(self, request, cleaned_data):
        ...
        if 'action' not in request.POST:
            ...
            assorole = TeamRole.objects.get(tournament=request.user.profile.tournament, type=TeamRole.ASSOCIATED)
            for att in ps:
                if not TeamMember.objects.filter(team=cleaned_data['team'], attendee=att).exists():
                    TeamMember.objects.create(team=cleaned_data['team'], attendee=att, role=assorole)
            ...
            for att in ps:
                ...
                if cre:
                    ...
                    if conflict_field:
                        try:
                            apv = att.attendeepropertyvalue_set.filter(property=conflict_field).last()
                            conflicting = set(apv.conflict_origins.all()) | ori
                        except:
                            conflicting = ori
                    else:
                    ...
                    try:
                        pj = att.active_user.possiblejuror_set.get(tournament=att.tournament, approved_by__isnull=False)
                        exp = pj.experience
                    except:
                        exp = Juror.EXPERIENCE_LOW
...
class TeamsPreview(FormPreview):
    ...
    def parse_params(self, request):
        ...
        team = ModelDeleteListField(queryset=Team.objects.filter(tournament=request.user.profile.tournament).prefetch_related('teammember_set','origin'))

        self.form = type("TeamsForm", (forms.Form,), {'teams':team})
    ...
    @method_decorator(permission_required('team.delete_team'))
    def done(self, request, cleaned_data):
    ...
    def parse_params(self, request, round_nr):
        ...
        for fight in ro.fight_set.all():
            ...
            for role in FightRole.ROLE_TYPE:
                ...
                if role[0]=='obs':
                    required=False
                f = forms.ModelChoiceField(queryset=TeamPlaceholder.objects.filter(tournament=trn),widget=forms.TextInput(), initial=initial, required=required)
        ...
        def clean(self):
            ...
            if len(new) != len(all):
                raise forms.ValidationError("The number of teams should match all the placeholder teams.")
        ...
        for fi in self.round.fight_set.all():
            ...
            for role in FightRole.ROLE_TYPE:
                if form.cleaned_data["fight-%d-%s"%(fi.pk,role[0])]:
                    fight['roles'].append((role[1],form.cleaned_data["fight-%d-%s"%(fi.pk,role[0])]))
            fights.append(fight)
...
class TemplatePreview(FormPreview):
    ...
    def parse_params(self, request, template_id):
        ...
        for room in rooms:
            fields["room-%d"%(room.id,)]=forms.CharField(max_length=30,initial=room.name, label="Room %s (capacity %d)"%(room.name, room.capacity()))
    ...
    def process_preview(self, request, form, context):
        ...
        phteams=TeamPlaceholder.objects.filter(tournament=request.user.profile.tournament)
        tpteams=list(sorted(list(set(
            TemplateAttendance.objects.filter(fight__round__template=self.template).values_list("team", flat=True)))))
        ...
        for round in TemplateRound.objects.filter(template=self.template):
            ...
            for fight in round.fights.all():
                ...
                for att in fight.templateattendance_set.all():
                    role={'role':att.get_type_display(),'team':phteams[tpteams.index(att.team)]}
                    fi["att"].append(role)
    ...
    def done(self, request, cleaned_data):
        ...
        if Round.objects.filter(tournament=trn).exists():
            msg = [Message(subject="Plan not empty",
                           level_tag='error')]

        elif self.template.teams_nr() != phteams.count():
            msg = [Message(subject="Number of teams not matching",
                           level_tag='error')]

        if len(msg) != 0:
            rounds = Round.selectives.filter(tournament=trn).order_by('order')
            scheds = ScheduleTemplate.objects.all()
            return render(request, "plan/overview.html",
                          context={'rounds': rounds, 'schedules': scheds, 'messages': msg})

        tpteams = list(sorted(list(set(
                                  TemplateAttendance.objects.filter(fight__round__template=self.template).values_list(
                                      "team", flat=True)))))

        for round in TemplateRound.objects.filter(template=self.template):

            ro = Round.objects.create(tournament=trn, order=round.order, type=Round.SELECTIVE)

            for fight in round.fights.all():
                room=Room.objects.get_or_create(tournament=trn,
                                           name=cleaned_data["room-%d" % (fight.room_id)])[0]
                room.save()
                ...
                for i in range(len(tplat)):
                    ...
                    for idx,att in enumerate(tplat):
                        role=FightRole.objects.get(tournament=trn,type=roleorder[idx].type)
                        StageAttendance.objects.create(stage=st, team_placeholder=phteams[tpteams.index(att.team)], role=role)
                    tplat = tplat[1:] + tplat[:1]

        Round.objects.create(tournament=trn, order=TemplateRound.objects.filter(template=self.template).count()+1, type=Round.FINAL)
    ...
    def parse_params(self, request):
        ...
        if not self.fr.fight_set.exists():
            ...
            for team in self.rank[-1]:
                f = forms.ChoiceField(choices=(('','----'),(1,1),(2,2),(3,3),(4,4)), required=False)
                if team['rank']<=3:
                    f.initial=team['rank']
            ...
            def teams(self):
                return [ (self.__getitem__("team-%d"%team['pk']), team) for team in self.ranking ]
    ...
    def done(self, request, cleaned_data):
        if not self.fr.fight_set.exists():
            ...
            for i in range(len(teams)):
                ...
                for idx, t in enumerate(teams):
                    StageAttendance.objects.create(stage=st, team = t,
                                                   role=FightRole.objects.get(tournament=self.trn, type=roles[idx]))
                teams = teams[1:] + teams[:1]

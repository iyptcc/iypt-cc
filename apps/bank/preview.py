...
class MergePreview(FormPreview):
    ...
    def parse_params(self, request, account):
        ...
        try:
            self.account = Account.objects.filter(id=account, owners__tournament=trn).distinct().get()
        except:
            raise PermissionError()

        accfield = forms.ModelChoiceField(queryset=Account.objects.filter(owners__tournament=trn).exclude(id=account).distinct())

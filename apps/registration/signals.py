...
def my_menuitems_builder(sender, **kwargs):
    ...
    if user.is_authenticated:
        ...
        if user.has_perm("registration.manage_data"):
            ...
            if req.session.get('menu-display-%d' % int(30),False) == True:
                if user.profile.tournament:
                    pdata, opts = persons_data(Attendee.objects.filter(id=user.profile.active.id))
                    for v in pdata[user.profile.active.id]['data']:
                        if "list" not in v:
                            v["list"]=[]
                    badges = []
                    reqir = len([x for x in pdata[user.profile.active.id]['data'] if x['value'] in [None, ''] and len(x['list'])==0 and ('image' not in x) and ('file' not in x) and x['required']])
                    if reqir > 0:
                        badges.append(("%s"%reqir,MenuItem.COLOR_RED))
                    opt = len([x for x in pdata[user.profile.active.id]['data'] if x['value'] in [None, ''] and len(x['list'])==0 and ('image' not in x) and ('file' not in x) and x['optional']])
                    if opt > 0:
        ...
        if user.has_perm("registration.accept_team"):
            opatr = {}
            if req.session.get('menu-display-%d' % int(30),False) == True:
                open_appl = Application.objects.filter(tournament=user.profile.tournament, origin__isnull=False).count()
                if open_appl > 0:
                    opatr={"badge":"%s"%open_appl, "badge_color":MenuItem.COLOR_YELLOW}
            t_listta = MenuItem(34, 'Team Applications', 'registration:list_team_applications', **opatr)
            profile_item.add_child(t_listta)
        if user.has_perm("registration.accept_role"):
            opatr = {}
            if req.session.get('menu-display-%d' % int(30),False) == True:
                open_appl = Application.objects.filter(tournament=user.profile.tournament, origin__isnull=True, team__isnull=True).count()
                if open_appl > 0:
                    opatr = {"badge": "%s" % open_appl, "badge_color": MenuItem.COLOR_YELLOW}
            t_listra = MenuItem(36, 'Role Applications', 'registration:list_role_applications',**opatr)

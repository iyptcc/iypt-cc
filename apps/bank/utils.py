        ...
        for pr in ass.attendee.roles.all():
            if pr.fee:
                fees.append(
                    {
                        "name": "%s fee for %s"
                        % (pr.get_type_display(), ass.attendee.full_name),
                        "amount": pr.fee,
                        "type": Payment.ROLE,
                        "role": pr,
                        "attendee": ass.attendee,
                    }
                )
    # property fee
    for att in team.members.all():
        for pf in PropertyFee.objects.filter(tournament=team.tournament):
            ...
            if alltrue and allfalse:
                fees.append(
                    {
                        "name": "%s fee for %s" % (pf.name, att.full_name),
                        "amount": pf.fee,
                        "type": Payment.PROPERTY,
                        "attendee": att,
                        "property": pf,
                    }
                )
...
def expected_person_fees(attendee):
    ...
    for pr in attendee.roles.all():
        if pr.fee:
            fees.append(
                {
                    "name": "%s fee for %s"
                    % (pr.get_type_display(), attendee.full_name),
                    "amount": pr.fee,
                }
            )
    # property fee
    for pf in PropertyFee.objects.filter(tournament=attendee.tournament):
        ...
        for trues in pf.if_true.all():
            try:
                apv = AttendeePropertyValue.objects.filter(
                    property=trues, attendee=attendee
                ).last()
                value = getattr(apv, apv.field_name[trues.type])
                if value != True:
                    alltrue = False
        ...
        for falses in pf.if_not_true.all():
            try:
                apv = AttendeePropertyValue.objects.filter(
                    property=falses, attendee=attendee
                ).last()
                value = getattr(apv, apv.field_name[falses.type])
                if value != False:
                    allfalse = False
        ...
        if alltrue and allfalse:
            fees.append(
                {
                    "name": "%s fee for %s" % (pf.name, attendee.full_name),
                    "amount": pf.fee,
                }
            )

...
class Team(models.Model):
    ...
    notify_applications = models.BooleanField(default=True, verbose_name="Notify Managers by Email for new Applications")
    ...
    class Meta:
        permissions = (
            ("view_teams", "Can list all team details"),
        )
    ...
    def get_students(self):
        return self.members.filter(Q(teammember__role__type=TeamRole.CAPTAIN) | Q(teammember__role__type=TeamRole.MEMBER)).prefetch_related("active_user__user")

    def get_leaders(self):
        return self.members.filter(teammember__role__type=TeamRole.LEADER).prefetch_related("active_user__user")

    def get_managers(self):
        return self.members.filter(teammember__manager=True).prefetch_related("active_user__user")


class StudentMemberManager(models.Manager):
    def get_queryset(self):
        return super(StudentMemberManager, self).get_queryset().filter(Q(role__type=TeamRole.MEMBER) | Q(role__type=TeamRole.CAPTAIN))

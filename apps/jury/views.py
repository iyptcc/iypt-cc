...
from .tasks import assignJob
from .utils import assignments_light, plan_from_db
...
class ListAssignments(View):
    ...
    def get(self, request):
        ...
        return render(request, "jury/assignments.html", context={"assignments":results,"form":form})
...
def assignJurors(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            res = assignJob.delay(trn.id, total_rounds=rounds,
                                  room_jurors=room_jurors,
                                  cooling_base=cooling_base, fix_rounds=fix_rounds)

            AssignResult.objects.create(tournament=trn, task_id=res.id, author=request.user.profile.active, total_rounds=rounds, room_jurors=room_jurors, cooling_base=cooling_base, fix_rounds=fix_rounds)
...
class JurorChange(generic.UpdateView):
    ...
    def get_object(self, queryset=None):
        obj = Juror.objects.get(id=self.kwargs['juror_id'], attendee__tournament=self.request.user.profile.tournament)
        return obj
...
def cost_graph(request, id):
    ...
    res = AsyncResult(id)
    get_object_or_404(AssignResult, task_id=res.id, tournament=request.user.profile.tournament)
    if res.successful():
        ...
        return render(request, "jury/cost_graph.html", context={"costs": costs,
...
class DisplayAssign(View):
    def get(self,request, id):
        ...
        if res.successful() and len(res.result[0])>0:
            ...
            assigned_jurors = JurorSession.objects.filter(fight__round__tournament=trn, fight__round__order__gt=ar.fix_rounds).exists()

            return render(request, "jury/assign.html", context={"uuid":res.id,
                                                                "rounds":plan,'assignments':assignment_sorted,
            ...
            if res.state == 'PROGRESS':
                ...
                try:
                    return render(request, "jury/progress.html",context={"progress":p, "costs":res.info["costs"]['total'][0::ticks], "best_costs":res.info["best_costs"][0::ticks], "total":res.info["total"]/ticks, "tickspace":ticks})
                except:
    ...
    def post(self, request, id):
        ...
        if not JurorSession.objects.filter(fight__round__tournament=trn, fight__round__order__gt=ar.fix_rounds).exists():
            if res.successful() and len(res.result[0]) > 0:
                ...
                nonvote_role = JurorRole.objects.get(type=JurorRole.NONVOTING, tournament=trn)
                ...
                for round in plan[ar.fix_rounds:]:
                    for fight in round:
                        ...
                        jurors = Juror.objects.filter(id__in=list(map(lambda x:x['id'],fight['jurors'])))
                        nonvoting = Juror.objects.filter(id__in=list(map(lambda x:x['id'],fight['nonvoting'])))

                        JurorSession.objects.create(juror=chair, fight=fobj, role=chair_role)
                        for juror in jurors:
                            JurorSession.objects.create(juror=juror, fight=fobj, role=juror_role)
                        for juror in nonvoting:
                            JurorSession.objects.create(juror=juror, fight=fobj, role=nonvote_role)
...
def jurorlist(request):
    ...
    jurors = Juror.objects.filter(attendee__tournament=request.user.profile.tournament).prefetch_related('availability',"attendee__active_user__user","conflicting")

    rounds=Round.objects.filter(tournament=request.user.profile.tournament)

    return render(request, "jury/persons.html", context={"persons":jurors, "rounds":rounds})
...
class JuryPlan(generic.ListView):
    ...
    def get_queryset(self):
        return Round.objects.filter(tournament = self.request.user.profile.tournament)\
            .prefetch_related("fight_set__room",
    ...
    def get_context_data(self, **kwargs):
        ...
        for round in Round.objects.filter(tournament=trn):
            ...
            freeJr['jurors'] = Juror.objects.filter(attendee__tournament=trn, availability=round)\
        ...
        unused = Juror.objects.filter(attendee__tournament=trn, availability__in=Round.objects.filter(tournament=trn)).distinct()
        for round in Round.objects.filter(tournament=trn):
            unused = unused.exclude(jurorsession__fight__in=round.fight_set.all(),jurorsession__role__type__in=[JurorRole.CHAIR,JurorRole.JUROR])
...
def genpdfjuryround(request, round_nr):
    ...
    if request.method == "POST":
        ...
        res = render_to_pdf.delay(trn.default_templates.get(type=Template.JURYROUND).id, pdf.id, context=context)
...
def genpdfjuryfeedback(request, fight_id):
    ...
    if request.method == "POST":
        ...
        fileprefix="jury-feedback-round-%d-room-%s-v"%(fight.round.order, fight.room.name)

        pdf = Pdf.objects.create(name="%s%d"%(fileprefix,_get_next_pdfname(trn, fileprefix)), tournament=trn)

        res = render_to_pdf.delay(trn.default_templates.get(type=Template.JURYFEEDBACK).id, pdf.id, context=context)
...
def jurorplan(request, juror_id):
    ...
    return render(request, "jury/juror.html", context={'juror':juror,'attendences':att})
...
class FightView(View):
    ...
    def get(self, request, fight_id):
        ...
        return render(request, "jury/fight.html", context={'round':fight.round.order,
...
class PdfJuryView(ObjectDownloadView):
    ...
    def get_object(self, queryset=None):
        try:
            obj = Round.objects.get(tournament=self.request.user.profile.tournament, order=self.kwargs['round']).pdf_juryplan
...
class PdfJuryFeedback(ObjectDownloadView):
    ...
    def get_object(self, queryset=None):
        try:
            obj = Fight.objects.get(round__tournament=self.request.user.profile.tournament, id=self.kwargs['fight_id']).pdf_jury_feedback
            if not obj.status in [Pdf.SUCCESS, Pdf.UPLOAD]:
                raise Pdf.DoesNotExist("File not yet available")
            return obj.file
        except:
...
@method_decorator(login_required, name='dispatch')
@method_decorator(permission_required('jury.delete_all_jurorsessions',raise_exception=False),name='dispatch')
class JuryClean(ConfirmedDeleteView):

    def get_objects(self, request, *args, **kwargs):
        trn = request.user.profile.tournament
        return JurorSession.objects.filter(fight__round__tournament=trn,fight__round__order__gt=kwargs["fix_rounds"])
...
class AcceptPossibleJuror(View):
    ...
    def get(self, request, id):
        ...
        pJ = get_object_or_404(PossibleJuror, id=id, tournament=request.user.profile.tournament, approved_at__isnull=True)

        context = application_propertyvalues(pJ.tournament, ParticipationRole.objects.get(type=ParticipationRole.JUROR, tournament=pJ.tournament),pJ.person)

        form = AcceptPossibleJurorForm(pJ)

        return render(request,"jury/accept_juror_application.html",context={"form":form, **context})

    def post(self,request, id):
        trn = request.user.profile.tournament

        pJ = get_object_or_404(PossibleJuror, id=id, tournament=trn, approved_at__isnull=True)
        ...
        if form.is_valid():
            ...
            if form.cleaned_data["notify"]:
                send_mail('Your application for %s as possible Juror was accepted' % (pJ.tournament.name, ),
                          'You are now eligible to be part of the jury at the tournament %s. You can now apply in your role to the tournament.' % ( pJ.tournament.name, ),
                          settings.EMAIL_FROM,
                          [pJ.person.user.email], fail_silently=False, )

            return redirect("jury:possiblejurors")

        return render(request,"jury/accept_juror_application.html",context={"form":form})

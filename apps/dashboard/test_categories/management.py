...
class ManagementTests(VerboseTestCase):
    ...
    def create_root(self, **kwargs):
        root = User.objects.get_or_create(
            username="root",
            email="fe-x@nlogn.org",
            is_staff=True,
            is_superuser=True,
            password="pbkdf2_sha256$30000$bfSRcz8435Km$VOJasXkSscLPajzWpi3An2aA8YKkn9keyX/FGassy/k=",
        )[0]
    ...
    def create_user(self, *args, **kwargs):
        r = self.client.post("/management/users/create", kwargs)
        print(r)
    ...
    def set_password(self, username, password):
        ...
        r = self.client.post(
            "/management/users/%d/password" % u.id, {"password": password}
        )
        self.assertIn(r.status_code, [200, 302])

    def add_persons_to_tournament(self, usernames, tournaments):
        trns = Tournament.objects.filter(slug__in=tournaments).values_list(
            "id", flat=True
        )
        print(trns)
        userids = User.objects.filter(username__in=usernames).values_list(
            "id", flat=True
        )
        print(userids)
        self._preview_post(
            ...
            {
                "tournaments": trns,
                "_add_tournaments": "add tournaments for selected",
                "persons": userids,
            },
            {"action": "_add_tournaments"},
        )

    def add_profile_property(self, name, type):
        r = self.client.post(
            "/management/profile/create/", {"name": name, "type": type}
        )
        self.assertIn(r.status_code, [200, 302])

...
class ListPreview(FormPreview):
    ...
    def parse_params(self, request):
        ...
        qs = self.get_queryset().filter(**self._filters)\
            ...
            for ff in filterfuncs:
                try:
                    ...
                    if len(inarg) > 0:
                        ...
                        for obj in qs:
                            if ff["filter_func"](obj) not in list(map(int,inarg)):
                                if obj.id in ids:
                                    ids.remove(obj.id)
                    ...
                    if len(exarg) > 0:
                        ...
                        for obj in qs:
                            if ff["filter_func"](obj) in list(map(int,exarg)):
                                if obj.id in ids:
                                    ids.remove(obj.id)
        ...
        self.obj_list = ModelDeleteListField(queryset=qs)
    ...
    def get_context(self, request, form):
        ...
        for fil in self.filters:
            ...
            filter_rest = "&".join(["&".join(["in_%s=%s"%(f,g) for g in self._filters[f]])
                                    for f in self._filters if f != fil['filter_name']])

            exclude_rest = "&".join(["&".join(["ex_%s=%s" % (f, g) for g in self._excludes[f]])
                                    for f in self._excludes if f != fil['filter_name']])

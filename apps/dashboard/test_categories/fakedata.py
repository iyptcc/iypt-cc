...
class FakeTeam:
    def __init__(self, locale=None, members=5):
        ...
        while not hasattr(self,"country"):
            try:
                self.country = pycountry.countries.get(alpha_2=locale.split('_')[1]).name
            except:
                locale = random.choice(list(AVAILABLE_LOCALES))
        ...
        self.teamleaders = [FakePerson(random.choice(['teamleader',"juror"]), locale) for i in range(2)]

    # maybe more stuff later

class FakeTournament:

    def __init__(self, host_locale, teams, team_members, indep_jur, local_jur):
        remaining = list(AVAILABLE_LOCALES)
        self.host_locale = host_locale
        self.host_country = pycountry.countries.get(alpha_2=host_locale.split('_')[-1]).name
        ...
        fake = Faker()
        for p in range(17):
            self.problems.append((p+1,fake.sentence(nb_words=4),"\n".join(fake.sentences())))

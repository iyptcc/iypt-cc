...
def assignAneal(tournament_id, total_rounds=1000, room_jurors=5, cooling_base=0.99, fix_rounds=0, task=None):
    ...
    def _copy(plan):
        ...
        for round in plan:
            ...
            for fight in round["fights"]:
                nfight = {"fight": fight['fight'], "jurors":copy.copy(fight["jurors"]), "nonvoting": copy.copy(fight['nonvoting'])}
    ...
    def _swap(plan, fix_rounds):
        ...
        def random_juror(plan, round_n):
            ...
            _jurors = len(plan[_round_n]["free"])+len(plan[_round_n]["fights"])*room_jurors
            ...
            if _juror_n < len(plan[_round_n]["free"]):
                ...
                _juror = plan[_round_n]["fights"][_fight_n]["jurors"][(_juror_n % room_jurors)]
        ...
        def check_hard(plan, src,dst):
            ...
            def check_fight_local(src, dst):
                ...
                if 'fight' in src:
                    if check_conflict_origin(src['fight'],dst['juror']):
                        return True
                    if not check_chair_in_fight(plan[src["round_n"]]["fights"][src["fight_n"]]):
                        return True
            ...
            def meet_often(fight, juror):
                if list(filter(lambda j: j.possible_chair, fight['jurors']))[0] != juror:
                    return False

                this_origins = set(
                    fight['fight'].stage_set.get(order=1).attendees.all().values_list("origin_id", flat=True))

                meets = 1
                for round in plan:
                    for f in round['fights']:
                        if list(filter(lambda j: j.possible_chair, f['jurors']))[0] == juror:
                            # is chair
                            fight_origins = set(
            ...
            if 'free' not in src:
                if meet_often(plan[src["round_n"]]["fights"][src["fight_n"]], dst["juror"]):
                    return False
            if 'free' not in dst:
                if meet_often(plan[dst["round_n"]]["fights"][dst["fight_n"]], src["juror"]):
                    return False
        ...
        def switch_juror(plan, src,dst):
            ...
            if "free" in dst:
                ...
                plan[dst["round_n"]]["fights"][dst["fight_n"]]["jurors"].remove(dst["juror"])
                plan[dst["round_n"]]["fights"][dst["fight_n"]]["jurors"].append(src["juror"])
        ...
        for round in trn.round_set(manager="selectives").all():
            ...
            if round.order <= fix_rounds:
                ...
                for fight in round.fight_set.all():
                    ...
                    fight_data['nonvoting'] = [js.juror for js in list(fight.jurorsession_set(manager="nonvoting").all())]
                ...
                jurors = list(jurors_qs.filter(availability=round, experience__gte=Juror.EXPERIENCE_LOW))

                jurors+=non_voted

                new_jurors = list(jurors_qs.filter(availability=round, experience=Juror.EXPERIENCE_NEW))
                #else:
                ...
                for fight in round.fight_set.all():
                    ...
                    # find a chair
                    for chair in chairs:
                        conflicting = set(chair.conflicting.all().values_list("id", flat=True))

                        possible = True
                        if chair in chair_meets:
                            for ori in list(participant_origins):
                                if ori in chair_meets[chair] and chair_meets[chair][ori] > 2:
                                    possible = False

                        if len(participant_origins & conflicting) == 0 and chair not in assigned_jurors and possible:
                            # check only twice:
                    ...
                    if len(fight_data['jurors']) < 1:
                        valid_plan = False
                        error.append("No chair for fight %s found"%fight)

                    # find the jurors
                    for i in range(room_jurors - 1):
                        for juror in jurors:
                            conflicting = set(juror.conflicting.all().values_list("id", flat=True))
                            if len(participant_origins & conflicting) == 0 and juror not in assigned_jurors:
                    ...
                    if len(fight_data['jurors']) < room_jurors:
                        valid_plan = False
                        error.append("Not 4 Jurors for fight %s found"%fight)

                    # assign nonvoting
                    for i in range(3):
                        for juror in new_jurors:
                            conflicting = set(juror.conflicting.all().values_list("id", flat=True))
                            if len(participant_origins & conflicting) == 0 and juror not in non_voted:
    ...
    chairs_qs = Juror.objects.filter(attendee__tournament_id=trn.id, possible_chair=True)
        ...
        if task:
            if i % 10 == 0:
                task.update_state(state='PROGRESS',
                                  meta={'current': i, 'total': total_rounds,
                                        'costs': cost_graph, "best_costs": best_cost_graph})
        else:
            if i % 100 == 0:

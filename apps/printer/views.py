...
from .forms import TemplateForm, TemplateNewForm, UploadForm
...
class FileView(ObjectDownloadView):
    ...
    def get_object(self, queryset=None):
        try:
            obj = self.request.user.profile.tournament.pdf_set.get(file=self.kwargs['name']).file
            return obj
        except:
    ...
    def post(self, request, id):
        ...
        if form.is_valid():
            ...
            if 'src' in form.changed_data:
                TemplateVersion.objects.create(template=template, author=request.user.profile, src=form.cleaned_data["src"])
            ...
            if "_render" in request.POST and form.cleaned_data['name']!="":
                ...
                pdf.task_id = res.id
                pdf.save()
                pdf.tags.add(PdfTag.objects.filter(tournament=trn, name="Debug").first())
            ...
            if "_source" in request.POST:
                ...
                src = render_template(template.id,context)
...
class PdfUpload(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                pdf = Pdf.objects.create(file=request.FILES['file'], name=form.cleaned_data['name'], status=Pdf.UPLOAD, tournament=request.user.profile.tournament )
...
class TagChange(UpdateView):
    ...
    def get_object(self, queryset=None):
        obj = PdfTag.objects.get(id=self.kwargs['id'], tournament=self.request.user.profile.tournament)
        return obj
...
class TagCreate(CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.add_message(self.request, messages.ERROR, "Tag type %s already exists"%form.instance.type)
...
class TagDelete(ConfirmedDeleteView):
    ...
    def get_objects(self, request, *args, **kwargs):
        obj = get_list_or_404(PdfTag, tournament=request.user.profile.tournament, id=kwargs["id"])
        return obj

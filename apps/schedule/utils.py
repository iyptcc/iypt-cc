...
def import_template(file):
    with open(file) as f:
        ...
        st = ScheduleTemplate.objects.get_or_create(name=sched['meta']['name'], teams=int(sched['meta']['teams']))[0]
        ...
        for rnr, round in enumerate(sched['rounds']):
            ...
            for fight in round:
                tf = TemplateFight.objects.get_or_create(room=TemplateRoom.objects.get(template=st,name=fight['name']),round=tr)[0]
                for type,n in TemplateAttendance.ROLE_TYPE:
                    if type in fight:
                        TemplateAttendance.objects.get_or_create(type=type,team=fight[type],fight=tf)

...
class NameValidator(Validator):
    ...
    def validate(self, nodes, *args, **kwargs):
        if "activeuser" in kwargs:
            if kwargs["activeuser"].user.first_name != "" and kwargs["activeuser"].user.last_name != "":
    ...
    def validate(self, nodes, *args, **kwargs):
        if "activeuser" in kwargs and "tournament" in kwargs:
            ...
            for ap in AttendeeProperty.user_objects.filter(Q(tournament=trn)):
                if not ( ap.apply_required.filter(type=ParticipationRole.STUDENT).exists() and ap.apply_required.filter(type=ParticipationRole.JUROR).exists() ):
                    continue
                t = ap.user_property.type
                up = ap.user_property

                valueo = UserPropertyValue.objects.filter(user=kwargs["activeuser"], property=up).last()
            ...
            raise AttributeError("ProfileValidator requires activeuser and tournament parameter")
...
class TeamManagerValidator(Validator):
    ...
    def validate(self, nodes, *args, **kwargs):
        if nodes[ProfileValidator.tag]=="finished":
            try:
                if kwargs["activeuser"].attendee_set.get(tournament=kwargs["tournament"]).roles.filter(type=ParticipationRole.TEAM_MANAGER):
                    return {self.tag:"finished"}
            except:
                pass
            return {self.tag:"available"}
...
class ApplicationWaitValidator(Validator):
    ...
    def validate(self, nodes, *args, **kwargs):
        if Application.objects.filter(applicant=kwargs["activeuser"], tournament=kwargs["tournament"],
                                      participation_role__type=self.get_roletype()).exists():
...
class PossibleJurorValidator(Validator):
    ...
    def validate(self, nodes, *args, **kwargs):
        if nodes[ProfileValidator.tag] == "finished":
            try:
                pj = kwargs["activeuser"].possiblejuror_set.get(tournament=kwargs["tournament"])
                return {self.tag: "finished"}
            except:
                pass
            return {self.tag: "available"}
...
class TeamRoleValidator(Validator):
    ...
    def validate(self, nodes, *args, **kwargs):
        if nodes[ProfileValidator.tag] == "finished":
            try:
                if kwargs["activeuser"].attendee_set.get(tournament=kwargs["tournament"]).roles.filter(
                        type=self.get_roletype()).exists():
                    return {self.tag: "finished"}
            except:
                pass
            return {self.tag: "available"}
...
class ExperiencedJurorValidator(Validator):
    ...
    def validate(self, nodes, *args, **kwargs):
        try:
            if kwargs["activeuser"].attendee_set.get(tournament=kwargs["tournament"]).roles.filter(
                        type=ParticipationRole.JUROR).exists():
                return {self.tag: "finished"}
        except:
            pass
        if Application.objects.filter(applicant=kwargs["activeuser"], tournament=kwargs["tournament"],
                                      participation_role__type=ParticipationRole.JUROR).exists():
            return {self.tag: "finished"}
        try:
            pj = kwargs["activeuser"].possiblejuror_set.get(tournament=kwargs["tournament"])
            if pj.approved_by:
...
class SetActiveValidator(Validator):
    ...
    depends_on = [TeamMemberWaitValidator, TeamManagerWaitValidator, VisitorWaitValidator, ExperiencedJurorWaitValidator]

    tags = ["active_experiencedjuror",
    "active_ioc",
    "active_loc",
    "active_manager",
    "active_role",
    ...
    def validate(self, nodes, *args, **kwargs):
        ...
        elif kwargs["activeuser"].attendee_set.filter(tournament=kwargs["tournament"]).exists():
...
class AssociateValidator(Validator):
    ...
    def validate(self, nodes, *args, **kwargs):
        if nodes[SetActiveValidator.tags[0]] == "finished":
            try:
                if kwargs["activeuser"].attendee_set.get(tournament=kwargs["tournament"]).teammember_set.count() > 0:
                    return {t : "finished" for t in self.tags}
                else:
                    return {t: "available"  for t in self.tags}
            except:
...
class DataValidator(Validator):
    ...
    def validate(self, nodes, *args, **kwargs):
        ...
        if kwargs["activeuser"].attendee_set.filter(tournament=kwargs["tournament"]).exists():

            atts = kwargs["activeuser"].attendee_set.filter(tournament=kwargs["tournament"])
            pdata, opts = persons_data(kwargs["activeuser"].attendee_set.filter(tournament=kwargs["tournament"]))

            for v in pdata[atts.first().id]['data']:
                if "list" not in v:
                    v["list"] = []
            reqir = len([x for x in pdata[atts.first().id]['data'] if
                         x['value'] in [None, ''] and len(x['list']) == 0 and ('image' not in x) and ('file' not in x) and x['required']])
            print("missing")
            print([x for x in pdata[atts.first().id]['data'] if
                         x['value'] in [None, ''] and len(x['list']) == 0 and ('image' not in x) and ('file' not in x) and x['required']])
            if reqir > 0:
                return {self.tag: "available"}
            else:

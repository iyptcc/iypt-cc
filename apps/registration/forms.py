...
from .models import ApplicationQuestion, ApplicationQuestionValue
from .utils import (
    field_for_property,
    field_for_question,
...
class AcceptRoleForm(forms.Form):
    ...
    def __init__(self, application, *args, **kwargs):
        ...
        self.fields["role"].queryset = ParticipationRole.objects.filter(
...
class DeclineRoleForm(forms.Form):
    ...
    def __init__(self, application, *args, **kwargs):
        ...
        self.fields["role"].initial = application.participation_role.name
...
class TeamSettingsForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["join_password"].label = (
            "Join Password (leave blank to keep existing password)"
        )
...
class ApplyForTeamForm(forms.Form):
    ...
    def clean(self):
        ...
        if team.join_password and password:
...
class AttendeePropertyForm(forms.Form):
    ...
    def __init__(self, attendee, read_profile=False, hidden=False, *args, **kwargs):
        ...
        if self.hidden:
            att_properties = AttendeeProperty.objects.filter(
                tournament=attendee.tournament
            )
        else:
            att_properties = AttendeeProperty.user_objects.filter(
                tournament=attendee.tournament
            )
        ...
        for ap in att_properties:
            ...
            try:
                ...
                depcheck = (
                    AttendeePropertyValue.objects.filter(
                        attendee=attendee, property=dep_ap
                    )
                    .last()
                    .bool_value
                )
            ...
            valueo = None
            try:
                valueo = AttendeePropertyValue.unconfirmed.filter(
                    attendee=attendee, property=ap
                ).last()
                if (not valueo) and read_profile and ap.user_property:
                    valueo = UserPropertyValue.objects.filter(
                        user=attendee.active_user, property=ap.user_property
                    ).last()
            ...
            if ap.manager_confirmed:
                try:
                    valueconf = AttendeePropertyValue.objects.filter(
                        attendee=attendee, property=ap
                    ).last()
                    if valueconf != valueo:
                        ls += ", confirmed value: %s" % getattr(
                            valueconf, valueconf.field_name[t]
                        )
                except:
                    pass
    ...
    def save(self, request):
        ...
        for ap in att_properties:
            if not (
                len(att.roles.all() & ap.required.all())
                or len(att.roles.all() & ap.optional.all())
                or ap.hidden == True
            ):
                continue
            ...
            try:
                apv = AttendeePropertyValue.objects.filter(
                    attendee=att, property=ap
                ).last()
            except:
                pass
            ...
            prelim = False
            if (
                ap.manager_confirmed
                and request.user.profile.active.roles.filter(
                    type=ParticipationRole.STUDENT
                ).exists()
            ):
                print("has to be confirmed")
                if not request.user.profile.active.teammember_set.filter(
                    manager=True
                ).exists():
...
class AcceptMemberForm(forms.Form):
    ...
    def __init__(self, application, *args, **kwargs):
        ...
        self.fields["email"].initial = application.applicant.user.email
        self.fields["role"].queryset = TeamRole.objects.filter(
            tournament=application.tournament
        )
...
class ApplyRoleForm(forms.Form):

    role = forms.ModelChoiceField(
        queryset=ParticipationRole.objects.none(), required=True, widget=Select2Widget()
    )

    def __init__(self, *args, **kwargs):
        tournament = kwargs["tournament"]
        user = kwargs["activeuser"]
        del kwargs["tournament"]
        del kwargs["activeuser"]
        super(ApplyRoleForm, self).__init__(*args, **kwargs)

        proles = ParticipationRole.objects.filter(
            ...
            type__in=[
                ParticipationRole.TEAM_LEADER,
                ParticipationRole.TEAM_MANAGER,
                ParticipationRole.STUDENT,
            ]
        )
        if not user.possiblejuror_set.filter(
            tournament=tournament, approved_at__isnull=False
        ).exists():
            proles = proles.filter(require_possiblejuror=False)

        self.fields["role"].queryset = proles


class ApplyRoleQuestionForm(forms.Form):
    def __init__(self, *args, **kwargs):
        tournament = kwargs["tournament"]
        ...
        del kwargs["activeuser"]
        del kwargs["role"]
        super(ApplyRoleQuestionForm, self).__init__(*args, **kwargs)

        for qu in ApplicationQuestion.objects.filter(role=self.role, active=True):
            self.fields["question-%d" % qu.id] = field_for_question(qu)
    ...
    def clean(self):
        ...
        for qu in ApplicationQuestion.objects.filter(role=self.role, active=True):
            if qu.required_if:
                if ("question-%d" % qu.required_if.id) in cleaned_data:
                    if cleaned_data["question-%d" % qu.required_if.id] == True:
                        if "question-%d" % qu.id in cleaned_data:
                            if cleaned_data["question-%d" % qu.id] == "":
                                raise forms.ValidationError(
                                    "Field %s is required, because you checked field %s"
                                    % (qu.name, qu.required_if.name)
                                )
                        else:
                            raise forms.ValidationError(
                                "Field %s is required, because you checked field %s"

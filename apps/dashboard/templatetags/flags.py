...
@register.filter
def flag_image_tuple(tuple):

    return mark_safe(
        '<div class="flag-container-72" style="display:inline;" data-toggle="tooltip" title="%s"><img class="flag-image-72" src="%s" /></div>'
        % (tuple[0], tuple[1])
    )
...
def flag_image(origin):
    if type(origin) == Origin:
        ...
        return mark_safe(
            '<div class="flag-container-72" data-toggle="tooltip" title="%s"><img class="flag-image-72" src="%s" /></div>'
            % (origin.name, flag_url(origin))
        )
